...
WEB_APP_URL = "여기에_복사한_URL을_입력하세요"
...
with tab1:
    ...
    if st.button("배팅 제출하기", use_container_width=True):
        df_data = get_data()
        if not name: st.error("이름을 입력해 주세요!")
        elif name in df_data["이름"].values: st.error("이미 배팅하셨습니다. '내 배팅 수정/취소' 탭을 이용하세요.")
        elif amount < 10000 or amount > 50000: st.error("배팅 금액은 10,000원에서 50,000원 사이로 입력해주세요.")
        else:
...
with tab2:
    ...
    df_data = get_data()
    if edit_name in df_data["이름"].values:
        user_row = df_data[df_data["이름"] == edit_name].iloc[0]
        ...
        new_amount = st.number_input("새로운 금액 (10,000 ~ 50,000 원):", min_value=10000, max_value=50000, value=int(user_row['배팅 금액']), step=5000)
        
        c1, c2 = st.columns(2)
        if c1.button("✏️ 배팅 정보 수정"):
            if new_amount < 10000 or new_amount > 50000: st.error("금액 범위를 확인해주세요.")
            else:
                requests.post(WEB_APP_URL, json={"action": "update", "name": edit_name, "team": new_team, "amount": new_amount})
                st.success("수정 완료!")
                st.rerun()
        if c2.button("❌ 배팅 취소 (삭제)"):
...
if not df_display.empty:
    total_pool = df_display["배팅 금액"].sum()
    st.metric("💰 총 누적 판돈", f"{total_pool:,} 원")
    
    # 데이터 계산 (콤마 및 예상 상금)
    team_sums = df_display.groupby("예측 우승팀")["배팅 금액"].sum()
    df_display["적중 시 예상 상금"] = df_display.apply(lambda row: int((total_pool / team_sums[row["예측 우승팀"]]) * row["배팅 금액"]), axis=1)
    
    display_df = df_display.copy()
    display_df["배팅 금액"] = display_df["배팅 금액"].map('{:,}원'.format)
    display_df["적중 시 예상 상금"] = display_df["적중 시 예상 상금"].map('{:,}원'.format)
