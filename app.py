...
with tab1:
    ...
    if st.button("배팅 제출하기"):
        ...
        elif a_in < 10000 or a_in > 50000: st.error(f"범위(1~5만원)를 벗어났습니다!")
        else:
...
with tab2:
    # [날짜 제한 설정] 7월 4일 23:59:59까지 수정/삭제 가능
    deadline = datetime(2026, 7, 4, 23, 59, 59)
    
    edit_name = st.text_input("조회할 이름", key="e_key")
    df = get_data()
    if not df.empty: df["배팅 금액"] = pd.to_numeric(df["배팅 금액"])
    ...
    if not df.empty and edit_name in df["이름"].values:
        ...
        if datetime.now() > deadline:
            st.error("⚠️ 수정/삭제 기간이 종료되었습니다. (7월 4일까지 가능)")
        else:
...
if not df.empty:
    ...
    summary["팀별_총_배팅액"] = summary["팀별_총_배팅액"].apply(lambda x: f"{x:,.0f}원")
